...
def load_currency_to_country():
    with open("C:/ITStudy/03_streamlit/pages/country.json", "r", encoding="utf-8") as file:
...
if not data.empty:
    ...
    if selected_data.empty:
        ...
        ax[0].set_xlabel('날짜', fontsize=12)
        ax[0].set_ylabel('매매 기준율 (KRW)', fontsize=12)
        ax[0].set_title(f"{selected_country} 환율 변동 (최근 1개월)", fontsize=14, fontweight='bold')
        ...
        ax[0].annotate(f'{max_value}', 
                       ...
                       xytext=(-10,0), 
                       ha='right', fontsize=12, fontweight='bold', color='black')
        ...
        # 두 번째 그래프: 변동률 막대 그래프
        colors = ["green" if x > 0 else "red" for x in analyzed_data["변동률 (%)"]]
        ...
        ax[1].set_ylabel("변동률 (%)", fontsize=12)
        ax[1].set_title(f"{selected_country} 환율 변동률 (상승/하락 구분)", fontsize=14, fontweight="bold")
